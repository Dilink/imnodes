...
// This is based on @thedmd code in suggestion on how to scale ImGui's draw lists.
...
void BeginNodeSelection(ImNodesEditorContext& editor, const int node_idx)
{
    ...
    editor.PrimaryNodeOffset =
        ref_origin + editor.Panning - GImNodes->MousePos;
...
void TranslateSelectedNodes(ImNodesEditorContext& editor, bool force)
{
    if (force || GImNodes->LeftMouseDragging)
    {
        ...
        const ImVec2 origin = SnapOriginToGrid(
            GImNodes->MousePos - editor.Panning +
            editor.PrimaryNodeOffset);
...
ImNodesStyle::ImNodesStyle()
    : GridSpacing(24.f), NodeCornerRounding(4.f), NodePadding(8.f, 8.f), NodeBorderThickness(1.f),
      ...
      MiniMapOffset(4.0f, 4.0f), ZoomSpeed(0.1f),
      Flags(ImNodesStyleFlags_NodeOutline | ImNodesStyleFlags_GridLines | ImNodesStyleFlags_Zoom),
      Colors()
{
}
...
void EndNodeEditor()
{
    ...
    if ((GImNodes->Style.Flags & ImNodesStyleFlags_Zoom) && ImGui::IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows))
    {
        const float zoom = ImNodes::EditorContextGetZoom() + ImGui::GetIO().MouseWheel * GImNodes->Style.ZoomSpeed;
